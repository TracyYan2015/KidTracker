...
bool SceneTest1::init()
{
    ...
    auto label = LabelTTF::create("SceneTest1", "Arial", 24);
...
// 	this->addChild(canvas, 0);
	...
	ArmatureDataManager::getInstance()->addArmatureFileInfo("an/role/elf_002/elf_002.ExportJson");
	
	Armature* arma = Armature::create("elf_002");
	arma->setPosition(Point(200.0f, 200.0f));
	this->addChild(arma);
	arma->getAnimation()->play("kkkkk");
...
void SceneTest1::playActionPointPath(Node* node, const std::vector<Point>& points, int index)
{
